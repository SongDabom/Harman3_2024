...
{
   if(huart == &huart2)  // BT input
   {
      if(strncmp(buf1,"LED",3) == 0)      //string , if you input "LED" on phone (3 = 3 character)
      {
      ...
      buf1[idx1++] = dum1;
      HAL_UART_Receive_IT(&huart2, &dum1, 1);
   ...
   else if(huart == &huart2) // KBD input
   {
      ...
      if(dum2 == '\r')
      {
         ...
         HAL_UART_Transmit(&huart1, "\0", 0, 10);
         idx2 = 0;
      }
      else buf2[idx2++] = dum2;
